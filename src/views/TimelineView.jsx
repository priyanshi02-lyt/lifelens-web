...
export default function TimelineView({ moments, chapters, onSelectMoment }) {
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-6 sm:px-8 lg:py-10 pb-24 md:pb-12">
      ...
      <div className="mb-5 flex items-center gap-2 overflow-x-auto pb-1 border-b border-slate-200">
        <button
      ...
      <div className="mb-8 flex gap-2 overflow-x-auto pb-2 scrollbar-none" data-testid="timeline-category-filters">
        {CATEGORIES.map(cat => (
          <button
            key={cat}
      ...
      <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        {/* Left Side: Chronological Moments Feed with Vertical Connecting Line */}
        <section className="relative" data-testid="timeline-list">
          {/* Vertical line running down */}
          <div className="absolute bottom-4 left-[76px] sm:left-[110px] top-4 w-px bg-[#dcdcf0]"></div>
          ...
          <div className="space-y-6">
            {filteredMoments.slice(0, 40).map((m, idx) => (
              <div
                ...
                className="group relative grid grid-cols-[68px_20px_1fr] sm:grid-cols-[96px_28px_1fr] gap-3 sm:gap-4 items-start cursor-pointer"
              ...
              >
                ...
                <div className="relative z-10 grid size-5 sm:size-7 place-items-center rounded-full bg-white ring-4 ring-[#faf8fc] border-2 border-[#5269dd] group-hover:scale-110 group-hover:bg-[#5269dd] group-hover:text-white transition mt-1.5">
                  <div className="size-1.5 sm:size-2 rounded-full bg-[#5269dd] group-hover:bg-white transition"></div>
                </div>

                {/* Moment Card Box */}
                <div className="rounded-[22px] bg-white p-4 sm:p-5 ring-1 ring-[#ece9f3] shadow-sm group-hover:shadow-md group-hover:border-[#5269dd]/40 transition">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <span 
                          className="size-7 grid place-items-center rounded-lg text-xs font-bold"
                          style={{ backgroundColor: m.color || '#e9e4ff' }}
                        >
                          {m.icon || '✦'}
                        </span>
                        <h3 className="font-heading text-base sm:text-lg font-bold text-[#2d1f3f] group-hover:text-[#5269dd] transition leading-snug">
                          {m.title}
                        </h3>
                      </div>
                    ...
                    <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 uppercase flex-shrink-0">
                      {m.mood}
                    </span>
                  </div>

                  {m.story && (
                    <p className="mt-3 text-xs sm:text-sm text-[#6f6782] leading-relaxed border-t border-slate-50 pt-2.5">
                      "{m.story}"
                    </p>
                  )}
        ...
        <aside className="space-y-6 hidden lg:block">
          <div className="sticky top-24 rounded-[28px] bg-white p-6 ring-1 ring-[#ece9f3] shadow-sm space-y-5">
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h4 className="font-heading text-sm font-bold text-[#2d1f3f] mt-0.5">{ch.title}</h4>
                      <p className="text-[11px] text-slate-500 mt-1 line-clamp-2">"{ch.quote}"</p>
                    </div>
                  </div>
                </div>
              ))}
